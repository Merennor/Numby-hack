...
public class RideStats extends Module {
    ...
    private void renderHorseNametag(AbstractHorse horseEntity, Entity entity) {
        ...
        if (displaySpeed.get())
            width += speedWidth;
        if (displayJumpHeight.get())
            width += jumpWidth;
        if (displayInventorySlots.get() && llama)
            width += invSlotsWidth;
        ...
        if (displayHealth.get())
            hX = text.render(healthText, hX, hY, Color.GREEN, true);
        if (displaySpeed.get())
            hX = text.render(speedText, hX, hY, Color.BLUE, true);
        if (displayJumpHeight.get() && !llama)
            text.render(maxJumpText, hX, hY, Color.GRAY, true);
        else if (displayJumpHeight.get() && llama)
            hX = text.render(maxJumpText, hX, hY, Color.GRAY, true);
        if (displayInventorySlots.get() && llama)
            text.render(invSlotsText, hX, hY, Color.YELLOW, true);
    ...
    public static double jumpStrengthToJumpHeight(double strength) {
        return -0.1817584952 * Math.pow(strength, 3) + 3.689713992 * Math.pow(strength, 2) + 2.128599134 * strength
                - 0.343930367;
    }
