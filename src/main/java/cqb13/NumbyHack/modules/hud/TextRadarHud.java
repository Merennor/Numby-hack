...
public class TextRadarHud extends HudElement {
    public static final HudElementInfo<TextRadarHud> INFO = new HudElementInfo<>(NumbyHack.HUD_GROUP, "text-radar",
            "Displays players and their stats in your visual range.", TextRadarHud::new);
    ...
    public void tick(HudRenderer renderer) {
        ...
        for (Player entity : players) {
            ...
            if (health.get())
                text += " " + (int) entity.getHealth() + entity.getAbsorptionAmount();
            if (ping.get())
                text += String.format("(%sms)", EntityUtils.getPing(entity));
            if (distance.get())
                text += String.format("(%sm)",
                        Math.round(Objects.requireNonNull(mc.getCameraEntity()).distanceTo(entity)));
    ...
    public void render(HudRenderer renderer) {
        ...
        for (Player entity : players) {
            ...
            if (health.get()) {
                ...
                double healthPercentage = Math.round(entity.getHealth() + entity.getAbsorptionAmount())
