...
public class ItemCounter extends HudElement {
    public static final HudElementInfo<ItemCounter> INFO = new HudElementInfo<>(NumbyHack.HUD_GROUP, "item-counter",
            "Count different items in text.", ItemCounter::new);
    ...
    public void render(HudRenderer renderer) {
        ...
        if (itemCounter.isEmpty()) {
            ...
            renderer.text(text, x + alignX(renderer.textWidth(text, shadow.get(), getScale()), alignment.get()), y,
                    TextHud.getSectionColor(0), shadow.get(), getScale());
        } else {
            for (String counter : itemCounter) {
                renderer.text(counter,
                        x + alignX(renderer.textWidth(counter, shadow.get(), getScale()), alignment.get()), y,
                        TextHud.getSectionColor(0), shadow.get(), getScale());
    ...
    private void updateCounter() {
        ...
        for (Item item : items.get())
            itemCounter.add(getName(item) + ": " + InvUtils.find(item).count());
